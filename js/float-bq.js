...
function Gravity(id){
  ...
  this.checkBound = function(){//猜测判断是不是按钮
    if (element.hasAttribute("data-bound")) {
      return element.dataset.bound === "true";
    }
  }
  
  this.checkBiaoQian = function(){//自定义标签
    if (element.hasAttribute("data-bq")) {
      return element.dataset.bound === "true";
    }
  }
  ...
  this.randomColor = function(){
	...
	/* 
		offsetTop
		offsetLeft 
		offsetWidth	内容+paddingLeft两边+borderLeft两边=盒子实际宽度-marginLeft
		offsetHeight 内容+paddingTop两边+borderTop两边=盒子实际高度-marginTop
	*/
 ...
 /* this.bounds = this.checkBiaoQian ? {//自定义标签
    min : {
      x : element.offsetLeft,
      y : element.offsetTop 
    },
  ...
  this.floatBackwards = function(){
    this.textSpans.forEach(function(t,i){
      ...
      t.style.transform = 'translateX('+ x +'px) translateY('+ y +'px)';//猜测为位置
...
//var paragraph = new Gravity('p-text');
var gravity = new Gravity('p-reset');
var button = document.getElementById('p-reset');//重置按钮
